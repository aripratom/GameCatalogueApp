package com.aripratom.core.data
...
import com.aripratom.core.data.source.remote.network.ApiResponse
...
import com.aripratom.core.utils.DataMapper
    ...
    override fun getGameDetail(id: String): Flow<Resource<Game>> =
            object : NetworkBoundResource<Game, GameResponse>() {
                override fun loadFromDB(): Flow<Game> {
                    return localDataSource.getGameDetail(id.toInt()).map {
                ...
                override suspend fun saveCallResult(data: GameResponse) {
                    val query = SimpleSQLiteQuery(
                            "UPDATE game SET description = ?, website = ? WHERE id = ?",
                            arrayOf(data.description, data.website, data.id)
                    )
